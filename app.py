...
[
  {{
    ...
    "apply_links": [
      {{"platform": "{sites_list[0]}", "url": "real search url with actual job title"}},
      {{"platform": "{sites_list[1] if len(sites_list) > 1 else sites_list[0]}", "url": "real search url with actual job title"}},
      {{"platform": "{sites_list[2] if len(sites_list) > 2 else sites_list[0]}", "url": "real search url with actual job title"}}
    ]
  }}
]

For apply_links URLs use these base URLs:
- LinkedIn: https://www.linkedin.com/jobs/search/?keywords=JOBTITLE&location={first_city}
- Rozee.pk: https://www.rozee.pk/job/jsearch/q/JOBTITLE
- Indeed: https://{indeed_domain}/jobs?q=JOBTITLE&l={first_city}
- Mustakbil: https://www.mustakbil.com/jobs/search/?search=JOBTITLE
- Bayt: https://www.bayt.com/en/international/jobs/search/?q=JOBTITLE
- GulfTalent: https://www.gulftalent.com/jobs?search=JOBTITLE
- Glassdoor: https://www.glassdoor.com/Job/jobs.htm?sc.keyword=JOBTITLE
- Reed: https://www.reed.co.uk/jobs/JOBTITLE-jobs
Replace JOBTITLE with actual job title words joined by +
...
with col_right:
    if uploaded_file and analyze_btn:
        ...
        if not cv_text.strip():
            ...
            # Job recommendations
            with st.spinner("Jobs dhundhe ja rahe hain..."):
                try:
                    ...
                    st.error("Error: Results parse nahi hue. Dobara try karein.")

...
def get_job_recommendations(cv_text, job_field, experience_level, location_pref, api_key):
    ...
    # Location ke hisab se salary currency aur job sites decide karo
    location_rules = {
        "UAE / Gulf": {
            "currency": "AED",
            "salary_example": "AED 8,000 - 18,000/month",
            "cities": "Dubai, Abu Dhabi, Sharjah, Riyadh, Doha, Kuwait City",
            "sites": ["LinkedIn", "Bayt", "GulfTalent", "Indeed"],
            "indeed_domain": "indeed.com",
            "note": "ALL 6 jobs MUST be based in Gulf countries (UAE, Saudi Arabia, Qatar, Kuwait, Bahrain, Oman). NO Pakistan jobs.",
        },
        "UK / Europe": {
            "currency": "GBP/EUR",
            "salary_example": "GBP 35,000 - 60,000/year",
            "cities": "London, Manchester, Berlin, Amsterdam, Paris, Dublin",
            "sites": ["LinkedIn", "Indeed", "Glassdoor", "Reed"],
            "indeed_domain": "uk.indeed.com",
            "note": "ALL 6 jobs MUST be based in UK or Europe. NO Pakistan jobs.",
        },
        "USA / Canada": {
            "currency": "USD/CAD",
            "salary_example": "USD 70,000 - 120,000/year",
            "cities": "New York, Toronto, San Francisco, Chicago, Vancouver",
            "sites": ["LinkedIn", "Indeed", "Glassdoor", "ZipRecruiter"],
            "indeed_domain": "indeed.com",
            "note": "ALL 6 jobs MUST be based in USA or Canada. NO Pakistan jobs.",
        },
        "Remote / Work from Home": {
            "currency": "USD",
            "salary_example": "USD 1,500 - 5,000/month",
            "cities": "Remote / Worldwide",
            "sites": ["LinkedIn", "Remote.co", "WeWorkRemotely", "Upwork"],
            "indeed_domain": "indeed.com",
            "note": "ALL 6 jobs MUST be fully remote positions. Include location as 'Remote - Worldwide'.",
        },
        "Karachi": {
            "currency": "PKR",
            "salary_example": "PKR 80,000 - 200,000/month",
            "cities": "Karachi only",
            "sites": ["LinkedIn", "Rozee.pk", "Indeed", "Mustakbil"],
            "indeed_domain": "pk.indeed.com",
            "note": "ALL 6 jobs MUST be based in Karachi, Pakistan ONLY.",
        },
        "Lahore": {
            "currency": "PKR",
            "salary_example": "PKR 70,000 - 180,000/month",
            "cities": "Lahore only",
            "sites": ["LinkedIn", "Rozee.pk", "Indeed", "Mustakbil"],
            "indeed_domain": "pk.indeed.com",
            "note": "ALL 6 jobs MUST be based in Lahore, Pakistan ONLY.",
        },
        "Islamabad": {
            "currency": "PKR",
            "salary_example": "PKR 80,000 - 200,000/month",
            "cities": "Islamabad / Rawalpindi only",
            "sites": ["LinkedIn", "Rozee.pk", "Indeed", "Mustakbil"],
            "indeed_domain": "pk.indeed.com",
            "note": "ALL 6 jobs MUST be based in Islamabad or Rawalpindi ONLY.",
        },
        "Pakistan (Any)": {
            "currency": "PKR",
            "salary_example": "PKR 60,000 - 200,000/month",
            "cities": "Any city in Pakistan",
            "sites": ["LinkedIn", "Rozee.pk", "Indeed", "Mustakbil"],
            "indeed_domain": "pk.indeed.com",
            "note": "Jobs can be from any city in Pakistan.",
        },
        "Worldwide": {
            "currency": "USD",
            "salary_example": "USD 2,000 - 8,000/month",
            "cities": "Any country worldwide",
            "sites": ["LinkedIn", "Indeed", "Glassdoor"],
            "indeed_domain": "indeed.com",
            "note": "Jobs can be from any country. Mix of international locations.",
        },
    }

    loc = location_rules.get(location_pref, location_rules["Pakistan (Any)"])

    # Apply links per platform
    apply_url_formats = {
        "LinkedIn":       "https://www.linkedin.com/jobs/search/?keywords={{title}}&location={{location}}",
        "Rozee.pk":       "https://www.rozee.pk/job/jsearch/q/{{title}}",
        "Indeed":         "https://{{indeed_domain}}/jobs?q={{title}}&l={{city}}",
        "Mustakbil":      "https://www.mustakbil.com/jobs/search/?search={{title}}",
        "Bayt":           "https://www.bayt.com/en/international/jobs/{{title}}-jobs/",
        "GulfTalent":     "https://www.gulftalent.com/jobs?search={{title}}",
        "Glassdoor":      "https://www.glassdoor.com/Job/jobs.htm?sc.keyword={{title}}",
        "Reed":           "https://www.reed.co.uk/jobs/{{title}}-jobs",
        "Remote.co":      "https://remote.co/remote-jobs/search/?search_keywords={{title}}",
        "WeWorkRemotely": "https://weworkremotely.com/remote-jobs/search?term={{title}}",
        "ZipRecruiter":   "https://www.ziprecruiter.com/jobs/search?search={{title}}",
    }

    sites_list = loc["sites"]
    indeed_domain = loc["indeed_domain"]
    first_city = loc["cities"].split(",")[0].strip().replace(" ", "+")

    apply_links_example = []
    for site in sites_list:
        fmt = apply_url_formats.get(site, "https://www.linkedin.com/jobs/search/?keywords={{title}}")
        apply_links_example.append({{"platform": site, "url_format": fmt}})

    prompt = f"""You are a strict job placement expert. Your task is to recommend jobs ONLY in the specified location.

STRICT RULE: {loc["note"]}
DO NOT recommend jobs outside this location under any circumstances.
...
- Location: {location_pref} ({loc["cities"]})
- Salary Currency: {loc["currency"]} (example: {loc["salary_example"]})

Return ONLY a valid JSON array of exactly 6 jobs. No markdown, no backticks, no explanation.
Every job's location field MUST be from: {loc["cities"]}
Salary MUST be in {loc["currency"]}.

JSON format:
[
  {{
    "title": "Exact Job Title",
    "location": "Specific city from {loc["cities"]}",
    "company_type": "Company type",
    "match_score": 88,
    "match_reason": "Why this matches the CV in 1-2 sentences",
    "key_skills_matched": ["skill1", "skill2", "skill3"],
    "skills_to_develop": ["skill1", "skill2"],
    "salary_range": "{loc["salary_example"]}",
    "job_type": "Full-time / Remote / Hybrid",
    "industry": "Industry",
    "description": "2-3 sentences about the role",
    "apply_links": [
      {{"platform": "{sites_list[0]}", "url": "https://www.linkedin.com/jobs/search/?keywords=Job+Title+Keywords&location={first_city}"}},
      {{"platform": "{sites_list[1] if len(sites_list)>1 else sites_list[0]}", "url": "https://{indeed_domain}/jobs?q=Job+Title&l={first_city}"}}
    ]
  }}
]

REMINDER: Location = {location_pref}. ALL jobs must be there. Salary in {loc["currency"]}."""
                    ...
                    for i, job in enumerate(jobs):
                        ...
                        <div class="job-card">
                            ...
                            <div style="color:#7878a0;font-size:13px;margin-bottom:12px;">
                                ...
                                📍 {job.get('location', location_pref)} &nbsp;•&nbsp;
